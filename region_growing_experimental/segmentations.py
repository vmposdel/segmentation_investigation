...
def simple_region_growing(img, seed, threshold=1, conn=4):
    ...
    if conn == 4:
        ...
        orient = [(2, 0), (2, 2), (0, 2), (-2, 2), (-2, 0), (-2, -2), (0, -2), (2, -2)] # 8 connectivity
    else:
    ...
    #Spreading
    while(dist<threshold and size<(pix_area + 1)):
        #adding pixels
        ...
        for j in range(len(orient)):
            reg[cur_pix[1] - 1, cur_pix[0] - 1] = 255
            reg[cur_pix[1] + 1, cur_pix[0] + 1] = 255
            reg[cur_pix[1] + 1, cur_pix[0] - 1] = 255
            reg[cur_pix[1] - 1, cur_pix[0] + 1] = 255
            reg[cur_pix[1], cur_pix[0] + 1] = 255
            reg[cur_pix[1], cur_pix[0] - 1] = 255
            reg[cur_pix[1] + 1, cur_pix[0]] = 255
            reg[cur_pix[1] - 1, cur_pix[0]] = 255
...
def define_seed(img, threshold=1, conn=4):
